...
def fetch_company_memos_with_joinedload(db: Session, company_id: int):
    ...
    query = (
        db.query(company_model)
        .options(joinedload(company_model.users).joinedload(user_model.memos))
...
def fetch_company_memos_with_join(db: Session, company_id: int):
    ...
    query = (
        db.query(company_model)
        .join(company_model.users)
        .join(user_model.memos)
        .filter(company_model.id == company_id)
    )
...
def fetch_company_memos_with_join_filter(
    db: Session, company_id: int, start_date: datetime = None, end_date: datetime = None
):

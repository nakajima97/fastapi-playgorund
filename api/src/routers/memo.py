...
from src.cruds.fetch_company_memos import (
    fetch_company_memos_with_joinedload,
    fetch_company_memos_with_join,
    fetch_company_memos_with_join_filter,
)
...
async def get_company_memo_with_joinedload(
    company_id: int = 1, db: Session = Depends(get_db)
):
...
async def get_company_memo_with_joined(
    company_id: int = 1, db: Session = Depends(get_db)
):
...
async def get_company_memo_with_joinedload_filter(
